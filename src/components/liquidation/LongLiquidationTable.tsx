...
import { LongLiquidationRow } from './LongLiquidationRow';

interface LongLiquidationTableProps {
  title: string;
  assets: LongLiquidationAsset[];
  icon: LucideIcon;
  bgColor: string;
  textColor: string;
...
export const LongLiquidationTable: React.FC<LongLiquidationTableProps> = ({
  title,
  assets,
  icon: Icon,
  bgColor,
  ...
  const totalPositions = assets.reduce((sum, asset) => sum + asset.longPositions, 0);
  const totalLiquidated = assets.reduce((sum, asset) => sum + asset.longLiquidated, 0);
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <div className="flex items-center space-x-4 text-sm text-gray-600">
                <span>{totalPositions} posições LONG</span>
                <span>${(totalLiquidated / 1e6).toFixed(1)}M liquidado LONG</span>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm max-w-xs">
                  Monitorando liquidações de posições longas em tempo real...
                </p>
              </div>
            </div>
          </div>
